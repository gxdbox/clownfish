...
// ===== Boss 胜利战利品（跑马灯式战利品机：外圈奖品格环绕滚动 + 中心暂停按钮，落在哪格就得哪奖） =====
...
export const BOSS_REWARD = {
    ...
    // —— 跑马灯参数（高亮格沿外圈顺时针循环，按暂停后减速停下） ——
    CELL_COUNT: 12,           // 外圈格子数（每个奖品至少占 1 格，剩余按权重分配）
    BASE_TICK: 0.1,           // 滚动每步时长(秒)——“跑马灯”基础节奏
    SLOW_STEPS: 5,            // 按暂停后再走几步才停（避免急停生硬，有“就差一点”的张力）
    SLOW_MULT: 1.34,          // 减速步长倍率（每步间隔 ×此值，逐渐变慢）
    MIN_ROLL: 0.7,            // 最短滚动时长(秒)（开局立刻按也要有滚动表现）
    AUTO_STOP: 6.0,           // 玩家不按时自动暂停(秒)（保证 Boss 后的流程一定继续）
};
